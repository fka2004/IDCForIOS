//
//  NotifyTableViewViewController.m
//  IDCForIOS
//
...
@interface NotifyTableViewViewController (){
...
@implementation NotifyTableViewViewController
...
- (void)refreshViewBeginRefreshing:(MJRefreshBaseView *)refreshView
{
    ...
    if (_header == refreshView) {
        if(currentPageNum != 1){
            ...
            [self.delegate getDate:currentPageNum searchText:nil tableView:self];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NotifyTableCell *cell = (NotifyTableCell *)[tableView dequeueReusableCellWithIdentifier:lab];
    
    if (cell == nil) {
        //系统自带cell
        //        cell = [[TableCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:lab];
        //自定义cell
        cell = [[[NSBundle mainBundle] loadNibNamed:@"NotifyTableCell" owner:self options:nil] lastObject];
    }
    Item *item = [_data objectAtIndex:indexPath.row];
    if(cell.cellTitle.text){
        cell.cellTitle.text = item.title;
    }
    if(cell.cellContent.text){
...
{
    NotifyTableCell *cell =[self tableView: tableView cellForRowAtIndexPath: indexPath];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //返回选中的item
    if ([self.delegate respondsToSelector:@selector(setCellOnClick:item:)]) {
        Item *item = [_data objectAtIndex:indexPath.row];
        [self.delegate setCellOnClick:indexPath item:item];
    }
}
//设置cell修改类型
