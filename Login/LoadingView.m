...
- (id)initIsLikeSynchro:(BOOL)isLikeSynchro withMsg:(NSString *)msg{
    if (isLikeSynchro) {
    ...
    if (self) {
        ...
        lblTitle = [[UILabel alloc] initWithFrame:CGRectMake(0, 40, 150, 40)];
...
+ (LoadingView *)shareLoadingView:(NSString *)msg{
    ...
    @synchronized(self){
        if (mLoadingView==nil) {
            mLoadingView = [[self alloc] initIsLikeSynchro:NO withMsg:msg];
        }
    }
    return mLoadingView;
}
