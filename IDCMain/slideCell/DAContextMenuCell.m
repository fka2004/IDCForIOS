...
- (void)setUp
{
//    self.actualContentView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 300, 50)];
    UIView *acview = [[UIView alloc]initWithFrame:CGRectMake(0, 0, 320, 40)];
    acview.backgroundColor = [UIColor greenColor];
    self.actualContentView = acview;
    ...
    self.editable = YES;
    self.moreOptionsButtonTitle = @" 添加  ";
    self.deleteButtonTitle = @"添加";
    ...
    if ([recognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        if (recognizer.state == UIGestureRecognizerStateBegan) {
            ...
            if (!self.contextMenuHidden || (velocity.x > 0. || [self.delegate shouldShowMenuOptionsViewInCell:self])) {
                ...
                NSLog(@"显示%d",CGRectGetWidth(self.bounds));
